...
def create_board_figure(pcb, bom_row, layer=pcbnew.F_Cu, orientation="Landscape"):
...
if __name__ == "__main__":
    ...
    boardbbox = pcb.ComputeBoundingBox(True)
    ...
    print("This board is at position {},{} and is {} wide and {} high ({})".format(boardxl * 1e-6,
        boardyl * 1e-6,
        boardwidth * 1e-6,
        boardheight * 1e-6,
        orientation))
    ...
    for layer in (pcbnew.F_Cu, pcbnew.B_Cu):
        ...
        with PdfPages(fname_out) as pdf:
            for i, bom_row in enumerate(bom_table):
                ...
                create_board_figure(pcb, bom_row, layer=layer, orientation=orientation)
                pdf.savefig()
                plt.close()
